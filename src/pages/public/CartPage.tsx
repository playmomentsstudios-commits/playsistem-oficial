...
export function CartPage(){
 ...
 <div className="grid lg:grid-cols-[1fr_320px] gap-6"><div className="space-y-3">{cart.items.map(i=><div key={i.productId} className="p-3 sm:p-4 rounded-2xl bg-[#141416] border border-white/10 grid grid-cols-[64px_1fr] sm:flex gap-3 sm:gap-4 items-center">{i.product.image?<img src={i.product.image} alt={i.product.name} className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-xl"/>:<div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-white/5 flex items-center justify-center">📦</div>}<div className="flex-1 min-w-0"><Link to={'/produtos/'+i.product.slug} className="font-semibold">{i.product.name}</Link><p className="text-sm text-gray-500">{money(i.price)}</p><div className="flex items-center gap-2 mt-3"><button onClick={()=>updateQuantity(i.productId,i.quantity-1)} className="w-10 h-10 rounded-xl bg-white/5">−</button><span className="w-8 text-center">{i.quantity}</span><button onClick={()=>updateQuantity(i.productId,i.quantity+1)} className="w-9 h-9 rounded-lg bg-white/5">+</button><button onClick={()=>removeItem(i.productId)} className="ml-auto sm:ml-3 min-h-10 px-2 text-xs text-red-400">Remover</button></div></div><b className="col-span-2 sm:col-span-1 text-right sm:text-left">{money(i.price*i.quantity)}</b></div>)}</div>
