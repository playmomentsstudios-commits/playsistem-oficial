...
import { customerDeletionApi } from '../../api/customerDeletion'
...
export function AdminCustomerDetailV2(){
  const {id=''}=useParams()
  ...
  async function analyzeDeletion(){
    ...
    setAnalyzingDelete(true)
    try{
      const result=await customerDeletionApi.preview(customer.id,deleteConfirmation)
      setDeleteSummary(result)
      setForcePhrase('')
    }catch(error:any){
      toast(error.message||'Não foi possível analisar a exclusão.','error')
    }finally{
      setAnalyzingDelete(false)
    }
  }

  async function deleteEmptyCustomer(){
    if(!customer)return
    setDeleting(true)
    try{
      await customerDeletionApi.removeEmpty(customer.id,deleteConfirmation)
  ...
  async function deleteCustomerAll(){
    if(!customer||forcePhrase.trim().toUpperCase()!=='EXCLUIR TUDO')return
    setDeleting(true)
    try{
      await customerDeletionApi.removeAll(customer.id,deleteConfirmation,forcePhrase)
      toast('Cliente e dados vinculados excluídos.','success')
      navigate('/admin/clientes',{replace:true})
    }catch(error:any){
      toast(error.message||'Não foi possível excluir todos os dados.','error')
    }finally{
      setDeleting(false)
    }
  }
      ...
      <div className="w-full max-w-lg max-h-[88vh] overflow-y-auto rounded-2xl bg-[#111113] border border-red-500/20 shadow-2xl p-5">
        <div className="flex justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Primeiro confira tudo que está ligado ao cliente. Projetos duplicados, financeiro, arquivos e conversas entram no resumo antes de qualquer exclusão.</p>
          </div>
        ...
        <div className="mt-5 p-3 rounded-xl bg-red-500/[0.06] border border-red-500/15">
          <p className="text-xs text-red-300">Digite exatamente o e-mail:</p>
          <p className="text-xs font-semibold mt-1">{customer.email}</p>
        </div>

        <input value={deleteConfirmation} onChange={e=>{setDeleteConfirmation(e.target.value);setDeleteSummary(null);setForcePhrase('')}} placeholder={customer.email} className="mt-4 w-full px-3 py-2.5 rounded-xl bg-black border border-white/10"/>

        {!deleteSummary&&<div className="flex justify-end gap-2 mt-5">
          ...
          <button type="button" disabled={analyzingDelete||deleteConfirmation.trim().toLowerCase()!==customer.email.trim().toLowerCase()} onClick={analyzeDeletion} className="px-4 py-2 rounded-xl bg-white/10 text-sm disabled:opacity-40">{analyzingDelete?'Analisando...':'Ver tudo que será afetado'}</button>
        </div>}

        {deleteSummary&&<div className="mt-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ['Projetos',deleteSummary.summary?.projects||0,'◆'],
              ['Pedidos',deleteSummary.summary?.orders||0,'▣'],
              ['Pagamentos',deleteSummary.summary?.payments||0,'◫'],
              ['Orçamentos',deleteSummary.summary?.quotes||0,'▤'],
              ['Arquivos',deleteSummary.summary?.files||0,'▰'],
              ['Conversas',deleteSummary.summary?.conversations||0,'●'],
              ['Mensagens',deleteSummary.summary?.messages||0,'◌'],
              ['Comprovantes',deleteSummary.summary?.receipts||0,'▧'],
            ].map(([label,value,icon])=><div key={String(label)} className="p-3 rounded-xl bg-white/[0.04] border border-white/8">
          ...
          <div className="mt-3 p-3 rounded-xl bg-white/[0.03] border border-white/8 flex justify-between gap-3 text-sm">
            <span className="text-gray-500">Total pago registrado</span>
            <b>{money(deleteSummary.summary?.paid_total||0)}</b>
          </div>

          {!deleteSummary.requires_force?<div className="mt-5">
            <p className="text-xs text-gray-500">Este cadastro não tem histórico operacional. A exclusão simples pode ser usada.</p>
            <div className="flex justify-end gap-2 mt-3">
              <button type="button" onClick={()=>setDeleteSummary(null)} className="px-4 py-2 rounded-xl bg-white/5 text-sm">Voltar</button>
              <button type="button" disabled={deleting} onClick={deleteEmptyCustomer} className="px-4 py-2 rounded-xl bg-red-600 text-sm">{deleting?'Excluindo...':'Excluir cadastro'}</button>
            </div>
          </div>:<div className="mt-5 rounded-xl border border-red-500/20 bg-red-500/[0.05] p-4">
            <h3 className="text-sm font-semibold text-red-300">Excluir cliente + todos os dados vinculados</h3>
            <p className="text-xs text-gray-500 mt-1">Remove os projetos desse cliente, pedidos, pagamentos, orçamentos, conversas, arquivos, comprovantes, pasta do Google Drive e a conta de acesso. Use principalmente para cadastros duplicados ou testes.</p>
            <p className="text-xs text-red-300 mt-4">Digite <b>EXCLUIR TUDO</b> para liberar a ação:</p>
            <input value={forcePhrase} onChange={e=>setForcePhrase(e.target.value)} placeholder="EXCLUIR TUDO" className="mt-2 w-full px-3 py-2.5 rounded-xl bg-black border border-red-500/20"/>
            <div className="flex justify-end gap-2 mt-4">
              <button type="button" disabled={deleting} onClick={()=>setDeleteSummary(null)} className="px-4 py-2 rounded-xl bg-white/5 text-sm">Voltar</button>
              <button type="button" disabled={deleting||forcePhrase.trim().toUpperCase()!=='EXCLUIR TUDO'} onClick={deleteCustomerAll} className="px-4 py-2 rounded-xl bg-red-600 text-sm disabled:opacity-40">{deleting?'Excluindo tudo...':'Excluir tudo'}</button>
            </div>
          </div>}
        </div>}
      </div>
